...
/*******************************************************************************
* Function Name: ReadArraySFlash
********************************************************************************
* Summary:
*        This routine reads an array of bytes (uint8) from SFlash memory, ensure 
*        a pointer to is provided to which data should be written.
*
* Parameters:
*  * data - Pointer to array of bytes to be read
*  len - Length of array
*
*******************************************************************************/    
void ReadArraySFlash(uint8 * data, uint16 len)
{   
    uint8 *sflashPtr;
    ...
    {
        data[dataIndex] = *sflashPtr;
        sflashPtr++;
    }
}

/*******************************************************************************
* Function Name: WriteArraySFlash
********************************************************************************
* Summary:
*        This routine writes an array of bytes (uint8) to SFlash memory using  
*        WriteUserSFlashRow method.
*
* Parameters:
...
*
*******************************************************************************/    
void WriteArraySFlash(uint8 data[], uint16 len)
{
    uint32 SFlash[USER_SFLASH_ROW_SIZE/4u];
    uint16 dataIndex;
    for(dataIndex=0; dataIndex<len/4u; dataIndex++)
    {
        SFlash[dataIndex] = data[dataIndex*4u+0u]     | /* Fill the user SFlash write buffer with known data */
                           (data[dataIndex*4u+1u]<<8u) | /* Convert 8bit DataSet to 32bit SFlash */ 
                           (data[dataIndex*4u+2u]<<16u)|
                           (data[dataIndex*4u+3u]<<24u);
    }
    WriteUserSFlashRow(0, &SFlash[0]); // Write to row 0
}
